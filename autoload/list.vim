...
function! list#colorize()
	...
	let l:output = system('od --read-bytes=20 --width=20 --format=u2 -An < /dev/urandom')

	" This is an incrementing counter for setting successive highlight groups.
	let l:n = 0

	" Set the highlight group for each random number.
	for l:num in split(output)
		exec "hi LIST" . l:n . " guifg=" . g:list_colors[l:num % len(g:list_colors)] . " guibg=NONE gui=NONE ctermfg=NONE ctermbg=NONE cterm=NONE"
		let l:n += 1
	endfor
endfunction
